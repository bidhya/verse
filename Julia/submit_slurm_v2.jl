...
function create_job(hpc, jobname, cores, memory, runtime, out_subfolder, start_idx, end_idx, valid_pix_count, begin_delay)
    ...
    open(job_file, "w") do f
        ...
        write(f, "#SBATCH --mail-user=jldechow@unc.edu\n")
        ...
        write(f, "julia verse/Julia/call_Blender_v2.jl $(out_subfolder) $(start_idx) $(end_idx)\n\n")  #  $(RES)
...
for i in StepRange(1, step, szY)
    ...
    if start_idx < 100  # needs most memory. old=120.
        pix_count_threshold = 65000  # 60000
    elseif start_idx < 2000
        pix_count_threshold = 85000  # 80000
    elseif start_idx < 3000
        pix_count_threshold = 95000  # 90000
    else
        pix_count_threshold = 120000  # 110000
    end
