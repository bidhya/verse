...
@info "Detected host" host_machine
...
elseif occursin("borg", host_machine) || occursin("discover", host_machine)  # TODO: discover
    ...
    default_submit_dir = joinpath("/discover/nobackup/projects/coressd", "Github", "slurm_jobs", string(water_year))
    slurm_submit_dir = get(ENV, "SLURM_SUBMIT_DIR", default_submit_dir)
    @info "Using Discover settings" slurm_submit_dir
    @info("SLURM_SUBMIT_DIR: $(get(ENV, "SLURM_SUBMIT_DIR", "NOT SET"))")
...
log_base = get(ENV, "SLURM_SUBMIT_DIR", default_submit_dir)
log_filename = joinpath(log_base, "OUT", "combine_ncfiles_$(water_year).log")
...
logger = FormatLogger(open(log_filename, "w")) do io, args
